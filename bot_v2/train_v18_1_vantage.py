...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V18_1_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v18_1.parquet")
...
def train_one(asset: str) -> dict:
    ...
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v18_1.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v18_1.json")
