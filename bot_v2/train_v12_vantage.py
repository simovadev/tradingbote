"""Train + validate ML V12 sur dataset Vantage 8 ans (mode PUR AMONT).

V12 vs V11 :
- Pas de feature snapshot_k (= marqueur du leakage V11)
- 1 ligne par OB au lieu de 3 (snapshots K=[3,7,15] supprimes)
- Le ML voit uniquement le contexte AMONT de l'OB (zero leakage)

Split temporel V12 (sur 8 ans Vantage 2018-03 -> 2026-05-21) :
...
Usage :
    python -m bot_v2.train_v12_vantage XAUUSD
    python -m bot_v2.train_v12_vantage --all
"""
...
# Memes seuils que V11. Dataset V12 ne contient PAS snapshot_k.
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V12_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v12.parquet")
    if final_path.exists():
        ...
        if not chunks:
            print(f"  Aucun dataset V12 trouve pour {asset} (ni final ni chunks)")
            return None
        print(f"  Chargement {len(chunks)} chunks V12 pour {asset}")
    ...
    if "snapshot_k" in df.columns:
        print(f"  WARNING : snapshot_k detecte dans le dataset V12, suppression")
...
def train_one(asset: str) -> dict:
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V12 (Vantage 8 ans, PUR AMONT) ===")
    print(f"{'='*60}")
    ...
    print(f"  Training LightGBM (config V11 'deeper')...")
    # Hyperparams identiques V11 pour permettre une comparaison nette V11/V12
    ...
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v12.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v12.json")
...
def main():
    p = argparse.ArgumentParser(description="Train + validate ML V12 sur Vantage (PUR AMONT)")
    ...
    if args.all or args.asset == "--all":
        ...
        print("\n" + "=" * 80)
        print("=== RECAP V12 VANTAGE (PUR AMONT) ===")
        print("=" * 80)
        ...
        recap_path = Path(f"{ROOT}/ml_metrics_v12_vantage_recap.json")
