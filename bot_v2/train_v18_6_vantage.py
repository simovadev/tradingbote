"""V18.6 : TRAIN REGULARISE FORT + ABLATION + WALK-FORWARD CV.
...
Output :
- ml_model_<ASSET>_vantage_v18_6.pkl (best variant)
- ml_features_<ASSET>_vantage_v18_6.json
- ml_metrics_v18_6_recap.json (avec wf_auc et auc_oos compares)

Usage :
    python -m bot_v2.train_v18_6_vantage --all
"""
...
# 9 nouvelles features V18.4 ajoutees - on les isole pour ablation
V18_4_NEW_FEATURES = {
...
def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge V18.4 (priorite) car contient TOUTES les features."""
...
def prepare_xy(df: pd.DataFrame, feature_set: str = "full"):
    """feature_set : 'core' (61 V18.3) ou 'full' (69 V18.4)."""
...
def train_variant(asset: str, df_closed: pd.DataFrame, feature_set: str) -> dict:
    ...
    # Walk-forward CV
    wf_mean, wf_std = walk_forward_auc(df_closed, feature_set, n_folds=5)
...
def train_one(asset: str) -> dict:
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V18.6 (regularise + ablation) ===")
    print(f"{'='*60}")
    ...
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v18_6.json")
...
def main():
    ...
    if args.all or args.asset == "--all":
        ...
        print("\n" + "=" * 110)
        print("=== RECAP V18.6 (REGULARISE + ABLATION + WF) ===")
        print("=" * 110)
        ...
        recap = Path(f"{ROOT}/ml_metrics_v18_6_recap.json")
