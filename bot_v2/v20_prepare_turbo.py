"""V20 PREPARE TURBO : sequences pour 78 actifs (sans asset_id utilise).

Difference vs v19_prepare_turbo :
- Lit champions_v20.parquet (78 actifs, 722k trades)
- asset_ids stocke mais ignore par V20 model
- Output : data/v20_chunks/<ASSET>.npz puis consolide en data/v20_dataset.npz
"""
...
# Memes 52 features que V19 (l'architecture V20 attend n_ict_features=52)
ICT_FEATURES = [
...
def extract_seqs_vectorized(trade_ts_arr, ohlcv_arr, ohlcv_ts_arr, n_bars):
    idx_array = np.searchsorted(ohlcv_ts_arr, trade_ts_arr, side="left")
...
def process_asset(asset, asset_id, data_dir_str, champions_path_str):
    ...
    try:
        ...
        # Stocke chunks en RAM via /dev/shm (96 GB) pour eviter saturation disque 32GB
        chunks_root = Path("/dev/shm/v20_chunks") if Path("/dev/shm").exists() else Path(ROOT) / "data" / "v20_chunks"
...
def main():
    print("=== V20 PREPARE TURBO : 78 workers, vectorise numpy ===\n")

    champions_path = ROOT / "data" / "champions_v20.parquet"
    if not champions_path.exists():
        print(f"FAIL : {champions_path} introuvable. Lance d'abord v20_extract_champions.py")
        return
    ...
    chunks_dir = Path("/dev/shm/v20_chunks") if Path("/dev/shm").exists() else ROOT / "data" / "v20_chunks"
    chunks_dir.mkdir(parents=True, exist_ok=True)
    print(f"Chunks dir : {chunks_dir}")

    # Skip actifs deja faits
    todo = []
    for asset_id, asset in enumerate(ALL_ASSETS):
    ...
    # Stats seulement (consolidation = trop gros pour np.savez sur 13GB)
    print("\n=== STATS ===")
    ...
    print(f"  Chunks pretes : {chunks_dir} (train les load directement)")
