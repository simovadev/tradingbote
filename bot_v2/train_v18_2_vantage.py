...
# V18.2 = 28 actifs avec leur propre config (chacune trouvee par genetique V4)
ALL_ASSETS = [
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V18_2_CHAMPION")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v18_2.parquet")
...
def train_one(asset: str) -> dict:
    ...
    # sklearn>=1.6 : cv="prefit" deprecated -> FrozenEstimator
    try:
    ...
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v18_2.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v18_2.json")
