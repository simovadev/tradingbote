"""Train + validate ML V14 sur dataset Vantage 8 ans (mode PUR AMONT).
...
- Maintenant (V14) : OB valide des que low <= ob_high (bullish) ou high >= ob_low (bearish).
  Latence 0-2 bougies. Coherent avec MARKET exec V12.

Build dataset avec OB_VALIDATION_MODE=mitigation (cf run_v14_parallel.py).
Tout le reste est strictement identique a V13 (hyperparams, split, 59 features dont
atr_regime par killzone).

Split temporel V14 (sur 8 ans Vantage 2018-03 -> 2026-05-21) :
...
Usage :
    python -m bot_v2.train_v14_vantage XAUUSD
    python -m bot_v2.train_v14_vantage --all
"""
...
# Memes seuils que V12. Dataset V14 ne contient PAS snapshot_k (mode PUR AMONT).
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V14_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v14.parquet")
    if final_path.exists():
        ...
        if not chunks:
            print(f"  Aucun dataset V14 trouve pour {asset} (ni final ni chunks)")
            return None
        print(f"  Chargement {len(chunks)} chunks V14 pour {asset}")
    ...
    if "snapshot_k" in df.columns:
        print(f"  WARNING : snapshot_k detecte dans le dataset V14, suppression")
...
def train_one(asset: str) -> dict:
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V14 (Vantage 8 ans, OB sans BOS (mitigation immediate)) ===")
    print(f"{'='*60}")
    ...
    # Hyperparams identiques V11/V12 pour comparaison nette V12/V14
    ...
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v14.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v14.json")
...
def main():
    p = argparse.ArgumentParser(description="Train + validate ML V14 sur Vantage (OB sans BOS (mitigation immediate))")
    ...
    if args.all or args.asset == "--all":
        ...
        print("\n" + "=" * 80)
        print("=== RECAP V14 VANTAGE (OB sans BOS (mitigation immediate)) ===")
        print("=" * 80)
        ...
        recap_path = Path(f"{ROOT}/ml_metrics_v14_vantage_recap.json")
