"""Genere les 13 wrappers de train V3.5 pour les autres actifs."""
from pathlib import Path

ASSETS = [
    "NAS100", "SP500", "DJ30",
    "GER40", "UK100", "FRA40", "JP225",
    "EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD", "USDCHF", "BTCUSD",
]

TEMPLATE = '''"""Train LightGBM {ASSET} V3.5 - Admiral 8 ans.
...
Output :
- bot_v2/ml_model_{ASSET}_admiral_v3_5.pkl
- ml_metrics_{ASSET}_admiral_v3_5.txt
"""
...
DATASET_PATH = Path(f"{{ROOT}}/data/ml_dataset_{ASSET}_admiral_8ans_V3_5.parquet")
MODEL_PATH = Path(f"{{ROOT}}/bot_v2/ml_model_{ASSET}_admiral_v3_5.pkl")
FEATURES_PATH = Path(f"{{ROOT}}/bot_v2/ml_features_{ASSET}_admiral_v3_5.json")
METRICS_PATH = Path(f"{{ROOT}}/ml_metrics_{ASSET}_admiral_v3_5.txt")
...
OUT = Path("c:/Users/Shadow/TradingBot/bot_v2")
for asset in ASSETS:
    path = OUT / f"ml_train_{asset}_admiral_8ans_V3_5.py"
    path.write_text(TEMPLATE.format(ASSET=asset), encoding="utf-8")
    print(f"OK : {path.name}")

print(f"\n{len(ASSETS)} train wrappers generes")
