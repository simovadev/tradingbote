"""Build dataset ML V11 sur donnees Vantage 8 ans (data_vantage/).
...
Usage :
    python -m bot_v2.build_v11_vantage_dataset XAUUSD
    python -m bot_v2.build_v11_vantage_dataset --all
"""
...
def build_one(asset: str):
    ...
    print(f"\n=== ML DATASET {asset} V11 (Vantage 8 ans, RR1.5 + sws1 + features + snapshots K=[3,7,15] (training realiste)) ===", flush=True)
    ...
    output_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v11.parquet")
    df_result = build_dataset(
        ...
        ltf="M1",
        version_suffix="_V11_VANTAGE",
    )

    print(f"\n>>> RECAP {asset} V11 : {len(df_result):,} candidats totaux", flush=True)
    if len(df_result) > 0 and "outcome" in df_result.columns:
        ...
        if len(closed) > 0:
            ...
            print(f">>> RECAP {asset} V11 : WR brut {wr:.1f}% sur {len(closed)} fermes",
                  flush=True)
    print("=" * 60, flush=True)


def main():
    p = argparse.ArgumentParser(description="Build V11 ML dataset (RR1.5 + sws1 + features + snapshots K=[3,7,15] (training realiste))")
