"""Train + validate ML V8 sur dataset Vantage 8 ans.

Pour chaque actif :
1. Charge le dataset V8 (data/ml_dataset_{asset}_vantage_v8.parquet ou chunks
   dans data/ml_partial_M1_V8_VANTAGE/).
...
Split temporel V8 (sur 8 ans Vantage 2018-03 -> 2026-05-21) :
  TRAIN : 2018-03 -> 2025-05-22 (~7.2 ans)
  VAL   : 2025-05-22 -> 2025-11-22 (6 mois)
  OOS   : 2025-11-22 -> 2026-05-21 (6 mois, jamais vu en training)

Usage :
    python -m bot_v2.train_v8_vantage XAUUSD
    python -m bot_v2.train_v8_vantage --all
"""
...
# V8 (2026-05-21) : split temporel 7 ans / 6 mois / 6 mois (8 ans Vantage)
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V8_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v8.parquet")
    if final_path.exists():
        ...
        if not chunks:
            print(f"  Aucun dataset V8 trouve pour {asset} (ni final ni chunks)")
            return None
        print(f"  Chargement {len(chunks)} chunks V8 pour {asset}")
...
def train_one(asset: str) -> dict:
    ...
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V8 (Vantage 8 ans, fix data leakage) ===")
    print(f"{'='*60}")
    ...
    # Save model + features
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v8.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v8.json")
...
def main():
    p = argparse.ArgumentParser(description="Train + validate ML V6 sur Vantage")
    ...
    if args.all or args.asset == "--all":
        ...
        # Save recap JSON
        recap_path = Path(f"{ROOT}/ml_metrics_v8_vantage_recap.json")
