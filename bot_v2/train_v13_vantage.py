"""Train + validate ML V13 sur dataset Vantage 8 ans (mode PUR AMONT).

V13 vs V12 :
- 1 seule feature change : `atr_regime` est maintenant calculee par killzone
  (moyenne ATR des 5 dernieres MEMES killzones) au lieu de "ATR(4h) / ATR(30j)".
- Conceptuellement plus adaptee a une strategie M1 ICT/SMC : la volatilite
  depend de la session (Asia/London/NY_AM/NY_PM/...), pas d'une moyenne 30j
  qui melange jours actifs et week-ends fermes.
- Permet de reduire N_BARS_M1 de 88k a ~8k (lookback max ~10 jours au lieu de 30).

Tout le reste est strictement identique a V12 (hyperparams, split, 58 autres features).

Split temporel V13 (sur 8 ans Vantage 2018-03 -> 2026-05-21) :
...
Usage :
    python -m bot_v2.train_v13_vantage XAUUSD
    python -m bot_v2.train_v13_vantage --all
"""
...
# Memes seuils que V12. Dataset V13 ne contient PAS snapshot_k (mode PUR AMONT).
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V13_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v13.parquet")
    if final_path.exists():
        ...
        if not chunks:
            print(f"  Aucun dataset V13 trouve pour {asset} (ni final ni chunks)")
            return None
        print(f"  Chargement {len(chunks)} chunks V13 pour {asset}")
    ...
    if "snapshot_k" in df.columns:
        print(f"  WARNING : snapshot_k detecte dans le dataset V13, suppression")
...
def train_one(asset: str) -> dict:
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V13 (Vantage 8 ans, atr_regime par killzone) ===")
    print(f"{'='*60}")
    ...
    print(f"  Training LightGBM (config V11/V12 'deeper')...")
    # Hyperparams identiques V11/V12 pour comparaison nette V12/V13
    ...
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v13.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v13.json")
...
def main():
    p = argparse.ArgumentParser(description="Train + validate ML V13 sur Vantage (atr_regime par killzone)")
    ...
    if args.all or args.asset == "--all":
        ...
        print("\n" + "=" * 80)
        print("=== RECAP V13 VANTAGE (atr_regime par killzone) ===")
        print("=" * 80)
        ...
        recap_path = Path(f"{ROOT}/ml_metrics_v13_vantage_recap.json")
