"""V18.7 BUILD FINAL — TOUS FIXES + configs par actif.
...
Output : data/ml_partial_M1_V18_7_FIXED/<asset>_M1_<date>.parquet
"""
import os
import sys
import json
...
df = build_dataset(
    ...
    ['{asset}'],
    output_path='{(ROOT / "data" / f"ml_dataset_{asset}_vantage_v18_7.parquet").as_posix()}',
    chunk_months=0.25, ltf='M1',
    version_suffix='_V18_7_FIXED',
)
...
"""
    try:
        ...
        # Save tail log
        log_path = ROOT / f"build_v18_7_{asset}.log"
        log_path.write_text("\n".join(out_lines[-20:]))
...
def main():
    print(f"=== V18.7 BUILD FINAL : {len(ASSETS)} actifs ===")
    ...
    for i, asset in enumerate(ASSETS, 1):
        ...
        (ROOT / "v18_7_build_results.json").write_text(
