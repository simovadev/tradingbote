"""Build dataset ML V9 sur donnees Vantage 8 ans (data_vantage/).
...
Usage :
    python -m bot_v2.build_v9_vantage_dataset XAUUSD
    python -m bot_v2.build_v9_vantage_dataset --all
"""
...
def build_one(asset: str):
    ...
    print(f"\n=== ML DATASET {asset} V9 (Vantage 8 ans, fix DATA LEAKAGES) ===", flush=True)
    ...
    output_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v9.parquet")
    df_result = build_dataset(
        ...
        ltf="M1",
        version_suffix="_V9_VANTAGE",
    )

    print(f"\n>>> RECAP {asset} V9 : {len(df_result):,} candidats totaux", flush=True)
    if len(df_result) > 0 and "outcome" in df_result.columns:
        ...
        if len(closed) > 0:
            ...
            print(f">>> RECAP {asset} V9 : WR brut {wr:.1f}% sur {len(closed)} fermes",
                  flush=True)
    print("=" * 60, flush=True)


def main():
    p = argparse.ArgumentParser(description="Build V9 ML dataset (fix DATA LEAKAGES)")
