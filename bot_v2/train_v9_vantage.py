"""Train + validate ML V9 sur dataset Vantage 8 ans.

Pour chaque actif :
1. Charge le dataset V9 (data/ml_dataset_{asset}_vantage_v9.parquet ou chunks
   dans data/ml_partial_M1_V9_VANTAGE/).
...
Split temporel V9 (sur 8 ans Vantage 2018-03 -> 2026-05-21) :
  TRAIN : 2018-03 -> 2025-05-22 (~7.2 ans)
  VAL   : 2025-05-22 -> 2025-11-22 (6 mois)
  OOS   : 2025-11-22 -> 2026-05-21 (6 mois, jamais vu en training)

Usage :
    python -m bot_v2.train_v9_vantage XAUUSD
    python -m bot_v2.train_v9_vantage --all
"""
...
# V9 (2026-05-22) : split temporel 7 ans / 6 mois / 6 mois (8 ans Vantage)
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V9_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v9.parquet")
    if final_path.exists():
        ...
        if not chunks:
            print(f"  Aucun dataset V9 trouve pour {asset} (ni final ni chunks)")
            return None
        print(f"  Chargement {len(chunks)} chunks V9 pour {asset}")
...
def train_one(asset: str) -> dict:
    ...
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V9 (Vantage 8 ans, fix DATA LEAKAGES) ===")
    print(f"{'='*60}")
    ...
    # Save model + features
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v9.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v9.json")
...
def main():
    p = argparse.ArgumentParser(description="Train + validate ML V9 sur Vantage")
    ...
    if args.all or args.asset == "--all":
        ...
        # Save recap JSON
        recap_path = Path(f"{ROOT}/ml_metrics_v9_vantage_recap.json")
