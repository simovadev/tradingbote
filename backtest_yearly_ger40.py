"""Backtest GER40 1 an OOS (user 2026-05-17 : test rapide avant decision live).

Meme logique que backtest_yearly_nas100 mais sur 1 an (vs 11 mois NAS100).
Seuil par defaut 0.60 (calibre sur metrics 4 ans : VAL 63.9% / TEST 51.4%).
"""
import sys
...
def load_ger40_model():
    with open('c:/Users/Shadow/TradingBot/bot_v2/ml_model_GER40.pkl', 'rb') as f:
        model = pickle.load(f)
    features = json.load(open('c:/Users/Shadow/TradingBot/bot_v2/ml_features_GER40.json'))['features']
    return model, features


def predict_ger40(model, features, r, ob, instrument):
...
def backtest_month(start, end, df_ltf_full, df_htf, df_htf2, df_d1, htf_swings,
    ...
    sws = get_param(INSTRUMENT, "swing_strength_m1", 3)
    ...
    for ob in obs_confirmed:
        ...
        if r.verdict != "TRADE" or r.trade_setup is None:
            continue
        proba = predict_ger40(model, features, r, ob, INSTRUMENT)
        if proba < threshold:
            continue
...
def main(threshold=0.60):
    print(f"=== BACKTEST GER40 1 AN (seuil {threshold}) ===\n", flush=True)
    ...
    model, features = load_ger40_model()

    # 1 an OOS : on prend les 12 derniers mois disponibles dans la data
    oos_end = df_ltf_full.index[-1].normalize().tz_convert("UTC")
    oos_start = oos_end - pd.Timedelta(days=365)
    print(f"Plage OOS : {oos_start.date()} -> {oos_end.date()}\n", flush=True)
    ...
    for idx, (start, end) in enumerate(months, 1):
        stats = backtest_month(start, end, df_ltf_full, df_htf, df_htf2, df_d1, htf_swings,
    ...
    print(f"\n=== BILAN GLOBAL GER40 (seuil {threshold}) ===")
