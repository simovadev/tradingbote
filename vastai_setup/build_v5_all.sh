#!/bin/bash
# Build V5 (cleanup filtres caches + features volatilite) pour les 14 actifs.
# Sur Vast.ai EPYC 9554/9654 96-128 cores -> ~6h total estime.
...
for ASSET in "${ASSETS[@]}"; do
    ...
    echo ""
    echo "========================================"
    echo "  BUILD $ASSET V5 ($(date +%H:%M:%S))"
    echo "========================================"
    python3 -m bot_v2.build_v5_dataset "$ASSET" 2>&1 | tail -20
    T1=$(date +%s)
    DUR=$((T1 - T0))
...
echo ""
echo "========================================"
echo "  ALL BUILDS V5 DONE in $((TOTAL - START))s"
echo "========================================"
