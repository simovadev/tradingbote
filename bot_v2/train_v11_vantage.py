"""Train + validate ML V11 sur dataset Vantage 8 ans.

Pour chaque actif :
1. Charge le dataset V11 (data/ml_dataset_{asset}_vantage_v11.parquet ou chunks
   dans data/ml_partial_M1_V11_VANTAGE/).
...
Split temporel V11 (sur 8 ans Vantage 2018-03 -> 2026-05-21) :
  TRAIN : 2018-03 -> 2025-05-22 (~7.2 ans)
  VAL   : 2025-05-22 -> 2025-11-22 (6 mois)
  OOS   : 2025-11-22 -> 2026-05-21 (6 mois, jamais vu en training)

Usage :
    python -m bot_v2.train_v11_vantage XAUUSD
    python -m bot_v2.train_v11_vantage --all
"""
...
# V11 (2026-05-22) : split temporel 7 ans / 6 mois / 6 mois (8 ans Vantage)
...
PARTIAL_DIR = Path(f"{ROOT}/data/ml_partial_M1_V11_VANTAGE")


def load_dataset(asset: str) -> pd.DataFrame | None:
    """Charge dataset final si dispo, sinon agrege les chunks partials."""
    final_path = Path(f"{ROOT}/data/ml_dataset_{asset}_vantage_v11.parquet")
    if final_path.exists():
        ...
        if not chunks:
            print(f"  Aucun dataset V11 trouve pour {asset} (ni final ni chunks)")
            return None
        print(f"  Chargement {len(chunks)} chunks V11 pour {asset}")
...
def train_one(asset: str) -> dict:
    ...
    print(f"\n{'='*60}")
    print(f"=== TRAIN {asset} V11 (Vantage 8 ans, fix DATA LEAKAGES) ===")
    print(f"{'='*60}")
    ...
    # V11 : config 'deeper' - gagnante au tuning (4 datasets x 4 configs).
    ...
    # Save model + features
    model_path = Path(f"{ROOT}/bot_v2/ml_model_{asset}_vantage_v11.pkl")
    features_path = Path(f"{ROOT}/bot_v2/ml_features_{asset}_vantage_v11.json")
...
def main():
    p = argparse.ArgumentParser(description="Train + validate ML V11 sur Vantage")
    ...
    if args.all or args.asset == "--all":
        ...
        # Save recap JSON
        recap_path = Path(f"{ROOT}/ml_metrics_v11_vantage_recap.json")
